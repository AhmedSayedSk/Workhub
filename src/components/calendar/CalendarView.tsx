...
const baseDefaultFormState: EventFormState = {
  title: '',
  description: '',
...
interface CalendarViewProps {
  /**
   * When provided, the calendar is scoped to a single project:
   * - only that project's events are shown
   * - the cross-project filter UI is hidden
   * - new events default to (and are locked to) this project
   */
  projectId?: string
}
...
export function CalendarView({ projectId }: CalendarViewProps) {
  ...
  const defaultFormState = useMemo<EventFormState>(
    () => ({ ...baseDefaultFormState, projectId: projectId || '' }),
    [projectId]
  )
  ...
  const accessibleEvents = useMemo(() => {
    let list = events
    if (!isAppOwner) {
      list = list.filter(e => e.projectId && accessibleProjectIds.has(e.projectId))
    }
    // When scoped to a project, only that project's events are visible.
    if (projectId) {
      list = list.filter(e => e.projectId === projectId)
    }
    return list
  }, [events, accessibleProjectIds, isAppOwner, projectId])
  ...
  const handleProjectFilterToggle = (pid: string) => {
  ...
  const calendarEvents: EventInput[] = accessibleEvents
    ...
    .filter(e => scoped || selectedProjectIds.length === 0 || selectedProjectIds.includes(e.projectId || ''))
    .map(event => {
  ...
  const filteredTableEvents = useMemo(() => {
    ...
    return filtered
  }, [accessibleEvents, selectedStatuses, selectedCategories, selectedProjectIds, tableSortField, tableSortDir, scoped])
  ...
  const handleSubmit = async () => {
    ...
    // When scoped to a project, new/edited events always belong to that project.
    const resolvedProjectId = scoped ? projectId : (form.projectId || undefined)
          ...
          <div className={cn(
            ...
            <div className="space-y-4 min-w-[14rem]">
              ...
              <div className="space-y-2">
                ...
                {/* Project Filter Popover — hidden when scoped to a single project */}
                {!scoped && eventProjectIds.length > 0 && (
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" size="sm" className="w-full justify-between text-sm">
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredTableEvents.length === 0 ? (
                <tr>
                  <td colSpan={scoped ? 5 : 6} className="text-center py-12 text-sm text-muted-foreground">No events found</td>
                </tr>
              ) : (
        ...
        <DialogContent className="max-w-5xl p-0 gap-0 overflow-hidden">
          ...
          <div className="flex max-h-[70vh] overflow-y-auto">
            ...
            <div className="w-2/5 bg-muted/10 border-l p-6 space-y-5">
              ...
              {/* Project Select — hidden/locked when scoped to a single project */}
              {!scoped && (
      ...
      {tooltip && (
        ...
        >
          <div className="bg-popover text-popover-foreground border rounded-lg shadow-lg p-3 max-w-[280px]">
            ...
            {!scoped && tooltip.event.projectId && (
